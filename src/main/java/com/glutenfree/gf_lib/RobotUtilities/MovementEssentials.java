package com.glutenfree.gf_lib.RobotUtilities;
...
import com.glutenfree.gf_lib.Debugging.ComputerDebugging;
import com.glutenfree.gf_lib.FTC_Classes.Point;
import com.glutenfree.gf_lib.FTC_Classes.Range;
import com.glutenfree.gf_lib.HelperClasses.CurvePoint;
import com.glutenfree.gf_lib.PiecewiseFunction.PiecewiseFunction;
import com.glutenfree.gf_lib.ReturnTypes.FloatPoint;

import static com.glutenfree.gf_lib.RobotUtilities.MovementVars.movement_turn;
import static com.glutenfree.gf_lib.RobotUtilities.MovementVars.movement_x;
import static com.glutenfree.gf_lib.RobotUtilities.MovementVars.movement_y;
import static com.glutenfree.gf_lib.RobotUtilities.MyPosition.AngleWrap;
import static com.glutenfree.gf_lib.RobotUtilities.MyPosition.subtractAngles;
import static com.glutenfree.gf_lib.RobotUtilities.MyPosition.worldAngle_rad;
import static com.glutenfree.gf_lib.RobotUtilities.MyPosition.worldXPosition;
import static com.glutenfree.gf_lib.RobotUtilities.MyPosition.worldYPosition;

public class MovementEssentials {
    public static double publicAngle;
    public static double publicTargetX;
    public static double publicTargetY;
    ...
    public static double movement_x_min = 0.19;
    ...
    public static double endingAngle = 0;
    public static void setLastAngle(double towerAngle) {
        endingAngle = towerAngle;
    }
...
//        double movement_y_power = (relative_y_to_point / (relative_abs_y+relative_abs_x));
        ...
        //new added 12/3/2019
        double movement_x_power = (relative_x_to_point / (Math.hypot(relative_abs_y,relative_abs_x)));
        double movement_y_power = (relative_y_to_point / (Math.hypot(relative_abs_y,relative_abs_x)));
    ...
    public static void allComponentsMinPower() {
    ...
    public static boolean followCurve(ArrayList<CurvePoint> allPoints, double followAngle, boolean allowSkipping){
        ...
        if(Math.abs(endingAngle) < 0.001) {
            ...
            pathExtended.add(new CurvePoint(pathExtended.get(pathExtended.size()-1)));
            pathExtended.set(pathExtended.size()-1,
                    extendAngle(endingAngle,allPoints.get(allPoints.size()-1).pointLength * 3,
                            allPoints.get(allPoints.size()-1)));
        }

        for(int i = 0; i < pathExtended.size()-1; i ++){
            ComputerDebugging.sendLine(new FloatPoint(pathExtended.get(i).x,pathExtended.get(i).y),
                    new FloatPoint(pathExtended.get(i+1).x,pathExtended.get(i+1).y));
        }
        ...
        //added stuff, 12/3/2019
        double tempFollowMeX = followMe.x;
        double tempFollowMeY = followMe.y;
        if(clipedDistToFinalEnd > 20) {
            double angle = Math.atan2(tempFollowMeY - worldYPosition, tempFollowMeX - worldXPosition);
            tempFollowMeX += 100 * Math.cos(angle);
            tempFollowMeY += 100 * Math.sin(angle);
        }


        gunToPosition(tempFollowMeX, tempFollowMeY, followAngle,
                followMe.moveSpeed,followMe.turnSpeed,
                followMe.slowDownTurnRadians,0,true);


        //so we can debug
        publicAngle = followAngle;
        publicTargetX = tempFollowMeX;
        publicTargetY = tempFollowMeY;




        //don't need anymore bc of the added thing
...
//                followMe.slowDownTurnRadians,0,true);
    ...
    private static CurvePoint extendAngle(double endingAngle, double extendLength, CurvePoint curvePoint) {
        ...
        double lineAngle = endingAngle + Math.toRadians(180);

        CurvePoint extended = new CurvePoint(curvePoint);
        extended.x += Math.cos(lineAngle) * extendLength;
        extended.y += Math.sin(lineAngle) * extendLength;
        return extended;
    }
...
//        currFollowAngle += subtractAngles(followAngle,Math.toRadians(90));
//
//        movementResult result = publicAngle(currFollowAngle,allPoints.get(followCurveIndex+1).turnSpeed,Math.toRadians(45));
//        movement_x *= 1 - Range.clip(Math.abs(result.turnDelta_rad) / followMe.slowDownTurnRadians,0,followMe.slowDownTurnAmount);
//        movement_y *= 1 - Range.clip(Math.abs(result.turnDelta_rad) / followMe.slowDownTurnRadians,0,followMe.slowDownTurnAmount);
//
//
